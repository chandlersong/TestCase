...
import org.commmstudy.quartz.job.ColorJob.ValueContainer;
...
public class StateJobExample extends AbstractJUnit4SpringContextTests {
    ...
    @Test
    public void testChangeValue() throws SchedulerException {
        log.info("------- test Simple --------");
        // get a "nice round" time a few seconds in the future....
        Date startTime = DateBuilder.nextGivenSecondDate(null, 10);
        // job1 will only run 5 times (at start time, plus 4 repeats), every 10 seconds
        ...
        ValueContainer contariner = new ValueContainer();

        job1.getJobDataMap().put(ColorJob.CHANGE_VALUE, contariner);

        log.info("------- change new Vaule to:" + newValue + " -----------------");
        Date scheduleTime1 = sched.scheduleJob(job1, trigger1);
        log.info(job1.getKey() + " will run at: " + scheduleTime1 + " and repeat: " + trigger1.getRepeatCount()
                + " times, every " + trigger1.getRepeatInterval() / 1000 + " seconds");
        ...
        try {
            ...
            for (int i = 0; i < 6; i++) {
                newValue = RandomStringUtils.randomAlphabetic(10);
                contariner.setValue(newValue);
